...
import classes from "./ForumDetail.module.css";
...
const ForumDetail = () => {
  ...
  const getData = async () => {
    ...
    const response = await Get(`${BaseURL("cms/page/singleForum")}`);
    if (response !== undefined) {
      setid(response?.data?._id);
      setpage(response?.data?.singleForum?.pageName);
      setimage(response?.data?.singleForum?.hero_image);
    }
    setloading(false);
  };
  ...
  const submitHandler = async () => {
    ...
    if (res !== undefined) {
      toast.success("Forum detail updated successfully!");
    }
    setloading(false);
  };
